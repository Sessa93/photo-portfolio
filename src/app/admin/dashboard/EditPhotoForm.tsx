...
const fields: {
  ...
  { name: "lens", label: "Lens", placeholder: "24-70mm f/2.8 GM" },
  {
    name: "settings",
    label: "Settings",
    placeholder: "f/8 · 1/250s · ISO 100",
  },
  { name: "film", label: "Film", placeholder: "Kodak Portra 400" },
  {
    name: "tags",
    label: "Tags",
    placeholder: "landscape, film, golden hour, mountains",
  },
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        {fields.map((f) => (
          <div
            key={f.name}
            className={
              f.name === "description" || f.name === "tags"
                ? "sm:col-span-2"
                : ""
            }
          >
              ...
              <div className="flex gap-2 items-center">
                ...
                <button
                  ...
                  onClick={async () => {
                    ...
                    try {
                      const locationInput = formRef.current!.elements.namedItem(
                        "location",
                      ) as HTMLInputElement;
                      const filmInput = formRef.current!.elements.namedItem(
                        "film",
                      ) as HTMLInputElement;
                      const lensInput = formRef.current!.elements.namedItem(
                        "lens",
                      ) as HTMLInputElement;
                      const settingsInput = formRef.current!.elements.namedItem(
                        "settings",
                      ) as HTMLInputElement;
                      ...
                      const data = await res.json();
                      if (res.ok) {
                        const tagsInput = formRef.current!.elements.namedItem(
                          "tags",
                        ) as HTMLInputElement;
